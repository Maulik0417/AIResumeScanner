...
# Initialize Sentence-BERT model for similarity comparison
...
def extract_keywords_yake(text):
    keyword_extractor = yake.KeywordExtractor(n=1, lan="en", dedupLim=0.9, top=100)
...
# Function to get SBERT embedding for text
def get_embedding(text):
    return sbert_model.encode(text, convert_to_tensor=True)

# Function to calculate cosine similarity between resume and job description's expanded keywords
def calculate_semantic_similarity(resume_text, job_description):
    resume_embedding = get_embedding(resume_text)
    
    # Extract keywords from job description
    job_keywords = extract_keywords_with_keybert(job_description)
    ...
    # Ensure keyword embeddings are not empty
    if len(keyword_embeddings) == 0:
        return jsonify({"error": "No keyword embeddings found."})

    # Convert list of embeddings to a tensor
    keyword_embeddings = torch.stack(keyword_embeddings)

    # Compute cosine similarity
    similarity_scores = util.cos_sim(resume_embedding, keyword_embeddings)
